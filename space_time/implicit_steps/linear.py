...
class LinearImplicitStep(implicit_steps.ImplicitStep):
    ...
    def inference_step(
        ...
        def proximal_cost(y, inner_x, inner_a):
            ...
            geom = PointCloud(y, inner_x, epsilon=self.epsilon)

            # Compute the Sinkhorn divergence.
            sink_div = sinkhorn_divergence(
                geom,
                x=geom.x,  # This is y
                y=geom.y,  # This is inner_x
                a=inner_a,  # y and inner_x have the same marginals
                b=inner_a,
                static_b=True,  # inner_x is static
                sinkhorn_kwargs={"max_iterations": self.sinkhorn_iter},
            ).divergence
        ...
        for _ in range(self.maxiter):
            y, state = opt.update(y, state, inner_x=x, inner_a=a)
            if self.wb:
